...
import 'package:proxy_network_image/proxy_network_image.dart';
...
class ProxyImage extends StatefulWidget {
  ...
  ProxyImage.network(
      String src, {
        ...
        this.proxy,
      }) : image = ProxyNetworkImage(src,proxy:proxy, scale: scale, headers: headers),
        assert(alignment != null),
        assert(repeat != null),
        assert(matchTextDirection != null),
        super(key: key);
...
class _ImageState extends State<ProxyImage> with WidgetsBindingObserver {
  ...
  @override
  void didUpdateWidget(ProxyImage oldWidget) {
